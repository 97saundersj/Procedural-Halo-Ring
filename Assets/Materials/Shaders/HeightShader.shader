Shader "Custom/Terrain" {
    Properties {
        ...
        _ShadowStrength("ShadowStrength", Float) = -1000
        _LightAngleThreshold("Light Angle Threshold", Float) = 82
    }
    SubShader {
        ...
        uniform float _LightAngleThreshold;
        ...
        void surf(Input IN, inout SurfaceOutputStandard o) {
            ...
            float3 lightDir = normalize(_WorldSpaceLightPos0.xyz); // Main directional light
            float angle = degrees(acos(dot(lightDir, IN.worldNormal)));

            // Darken based on light angle
            if (_LightAngleThreshold <= angle) {
				o.Albedo *= _ShadowStrength;
                //float blendFactor = _ShadowStrength; // Adjust this value to control the darkness
                //o.Albedo = lerp(o.Albedo, o.Albedo * blendFactor, angle / _LightAngleThreshold);
            }
			*/
        }
